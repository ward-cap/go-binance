...
func parseDepth(data []byte) (*DepthResponse, error) {
	...
	if err := jsonCodec.Unmarshal(data, &payload); err != nil {
		return nil, err
	}
	res := &DepthResponse{LastUpdateID: payload.LastUpdateID}
...
func parseKlines(data []byte) ([]*Kline, error) {
	var rows []klineTuple
	if err := jsonCodec.Unmarshal(data, &rows); err != nil {
		return nil, err
	}
...
func parseKline(row klineTuple) (*Kline, error) {
